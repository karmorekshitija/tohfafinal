...
function imageFileFilter(req, file, cb) {
  ...
  if (dangerousExts.includes(ext) || !allowedExts.includes(ext)) {
    return cb(new Error('Only JPEG, PNG, and WebP images (.jpg, .jpeg, .png, .webp) are allowed.'), false);
  }
...
// Resilient multi-image uploader that tries Cloudinary first, and seamlessly falls back
// to an optimized base64 Data URI if Cloudinary credentials or signatures fail.
function createResilientMultiUploader(fieldName, defaultFolder, maxCount = 10, options = {}) {
  ...
  return (req, res, next) => {
    memUpload(req, res, async (err) => {
      ...
      const folder = defaultFolder || 'general';
      ...
      for (let i = 0; i < req.files.length; i++) {
          ...
          try {
            const uploadOptions = {
              ...
              transformation: options.transformation || [
                { width: options.maxWidth || 1920, height: options.maxHeight || 1920, crop: 'limit', quality: 'auto:good', fetch_format: 'auto' }
              ],
            };
            ...
            file.path = optimizeCloudinaryUrl(uploadResult.secure_url);
        ...
        file.path = `data:${mime};base64,${base64Data}`;
        file.secure_url = file.path;
        file.url = file.path;
...
const uploadProductImages = createResilientMultiUploader('images', 'products', 8);

// Profile photo uploader (1 file)
const uploadProfilePhoto = createResilientSingleUploader('photo', 'profiles', {
  maxWidth: 400,
  maxHeight: 400,
  crop: 'fill',
});
...
const uploadRefImages = createResilientMultiUploader('images', 'customization-refs', 5);
...
const uploadProofImage = createResilientSingleUploader('proof', 'customization-proofs', {
  maxWidth: 1920,
  maxHeight: 1920,
});
