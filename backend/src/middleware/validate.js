...
const schemas = {
  ...
  register: Joi.object({
    ...
    daily_capacity_min:  Joi.number().integer().min(0).optional().allow(null),
    daily_capacity_max:  Joi.number().integer().min(0).optional().allow(null),
    instagram_handle:    Joi.string().trim().max(100).optional().allow('', null),
    instagram_followers: Joi.string().trim().max(100).optional().allow('', null),
    pickup_address:      Joi.object().optional().allow(null),
    pan_number:          Joi.string().trim().max(20).optional().allow('', null),
    gst_number:          Joi.string().trim().max(20).optional().allow('', null),
    portfolio_images:    Joi.array().items(Joi.string()).optional(),
  }).or('name', 'full_name'),
  ...
  createProduct: Joi.object({
    ...
    category_id:                 Joi.alternatives().try(Joi.string(), Joi.number(), Joi.object()).optional().allow(null, ''),
    categoryId:                  Joi.alternatives().try(Joi.string(), Joi.number(), Joi.object()).optional().allow(null, ''),
    subcategory_id:              Joi.alternatives().try(Joi.string(), Joi.number(), Joi.object()).optional().allow(null, ''),
    subcategoryId:               Joi.alternatives().try(Joi.string(), Joi.number(), Joi.object()).optional().allow(null, ''),
    category:                    Joi.alternatives().try(Joi.string(), Joi.number(), Joi.object()).optional().allow(null, ''),
    ...
    preparation_days:            Joi.number().integer().min(0).default(2),
    weight_grams:                Joi.number().min(1).default(500),
    is_customizable:             Joi.alternatives().try(Joi.boolean(), Joi.number(), Joi.string()).optional(),
    customization_mode:          Joi.string().valid('none', 'fixed', 'open').default('none'),
    customization_schema:        Joi.alternatives().try(Joi.object(), Joi.array(), Joi.string()).optional().allow(null, ''),
    images:                      Joi.alternatives().try(Joi.array().items(Joi.alternatives().try(Joi.string(), Joi.object())), Joi.string()).optional(),
    photos:                      Joi.alternatives().try(Joi.array().items(Joi.alternatives().try(Joi.string(), Joi.object())), Joi.string()).optional(),
    ...
    occasions:                   Joi.alternatives().try(Joi.array().items(Joi.string()), Joi.string()).optional(),
    occasion_tags:               Joi.alternatives().try(Joi.array().items(Joi.string()), Joi.string()).optional(),
    variants:                    Joi.alternatives().try(Joi.array().items(Joi.object()), Joi.string()).optional(),
    tags:                        Joi.alternatives().try(Joi.array().items(Joi.string()), Joi.string()).optional(),
    special_packaging_available: Joi.boolean().optional(),
    discount_percentage:         Joi.number().min(0).max(100).optional().allow(null),
    sale_price:                  Joi.number().min(0).optional().allow(null),
    seller_id:                   Joi.alternatives().try(Joi.string(), Joi.number()).optional().allow(null, ''),
  }).unknown(true),
  ...
  }).or('line1', 'address_line1', 'address_line'),
  ...
  openCustomizationConfig: Joi.object({
    product_id:         Joi.alternatives().try(Joi.string().uuid(), Joi.number(), Joi.string()).optional(),
    allowed_types:      Joi.alternatives().try(Joi.array().items(Joi.string()).min(1), Joi.string().min(1)).required(),
